Feature: KFS Fiscal Officer Account Copy
  ...
  Scenario: Copy C&G Account (Smoke Test)
    ...
    When  I start a copy of a Contracts and Grants Account
...
#    And Verify new attributes and CFDA# persist: Invoice frequency, Invoice type code, eVerify indicator, Cost Share for Project Number
    And   I update the Account with the following changes:
      ...
      | Appropriation Number          | Same as Original     |
      | Labor Benefit Rate Category   | Same as Original     |
      | Major Reporting Category      | Same as Original     |
      | Subfund Program               | Same as Original     |
...
#    And Verify FYI to Account Supervisor
    ...
    Then  the Account document has transactions matching the accounting lines from the Distribution Of Income And Expense document
...
#        Likely object code 9070 or 9080.
    And   the recovery Accounts specified in the Distribution Of Income And Expense document have posted income transactions
